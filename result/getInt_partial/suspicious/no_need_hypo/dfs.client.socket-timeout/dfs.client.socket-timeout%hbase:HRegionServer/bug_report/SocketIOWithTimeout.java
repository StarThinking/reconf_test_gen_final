...
abstract class SocketIOWithTimeout {
  ...
  int doIO(ByteBuffer buf, int ops) throws IOException {
    ...
    while (buf.hasRemaining()) {
      ...
      try {
        int n = performIO(buf);
        LOG.info("msx-debug doIO " + ops + " " + timeout + ",  performIO returned " + n + ", ch: " + channel);
        if (n != 0) {
      ...
      int count = 0;
      try {
        LOG.info("msx-debug doIO " + ops + " " + timeout + ", before select, ch: " + channel);
        Long begin_select = System.currentTimeMillis();
        count = selector.select(channel, ops, timeout);  
        Long end_select = System.currentTimeMillis();
        LOG.info("msx-debug doIO " + ops + " " + timeout + ", select for " + (end_select-begin_select) + " ms, returned " + count + ", ch: " + channel);
      ...
      if (count == 0) {
        LOG.info("msx-debug doIO " + ops + " " + timeout + ", select returned 0 and it is timeout " + timeout + ", ch: " + channel);
