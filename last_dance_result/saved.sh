...
eu_dir=~/vm_images/the_final/last_dance_result/; for i in *; do p_and_c="$($eu_dir/extract_utility.sh $i 'para')%$($eu_dir/extract_utility.sh $i 'comp')"; if [ ! -d $p_and_c ]; then mkdir $p_and_c; fi; mv $i $p_and_c; done
